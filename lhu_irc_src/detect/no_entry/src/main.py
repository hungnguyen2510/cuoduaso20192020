...
import numpy as np
import imutils
from bans_tfsign import BansTFSign
import time
...
#----------------------------- MAIN ---------------------------------#
rospy.init_node('run_no_entry_detect', anonymous=True)
...
pub_bans_sign = rospy.Publisher('/no_entry', Int32, queue_size=1)
tmp = 0
while not rospy.is_shutdown():
    if frame is not None:
        bans_sign = bans.detect_red_sign(frame.copy()) #MID
        if bans_sign != tmp:
            print("publish")
            # pub_bans_sign.publish(bans_sign)
            tmp = bans_sign
    rospy.sleep(0.001)